...
def test_change_label_config_repeater(tasks_count, annotations_count, predictions_count, business_client, project_id) -> None:
...
@pytest.mark.django_db
def test_config_validation_for_choices_workaround(business_client, project_id) -> None:
    """
...
@pytest.mark.django_db
def test_config_validation_for_missing_to_name_in_number_tag_fails(business_client, project_id) -> None:
    """
...
@pytest.mark.django_db
def test_parse_wrong_xml(business_client, project_id) -> None:
    # Change label config to Repeater
    payload = {
        'label_config': '<View> <Repeater on="$images" indexFlag="{{idx}}"> <Image name="page_{{idx}}" value="$images" maxWidth="100%"/>     <Header value="Utterance Review"/>     <RectangleLabels name="labels_{{idx}}" toName="page_{{idx}}">       <Label value="Header" hotkey="1"/> <Label value="Body" hotkey="2"/> <Label value="Footer" hotkey="3"/> </RectangleLabels> </Repeater> </View>'
    }
...
@pytest.mark.django_db
def test_label_config_versions(business_client, project_id) -> None:
